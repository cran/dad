...
#
# On ?crira une fonction df2folder() qui prendra en argument un data.frame
  ...
  function(x1, x2 = NULL, ..., cols.select = "intersect", rows.select = "") 
  {
    ...
    switch(class.arg,
           ...
           l = {
             ...
             # If the list have no names:
             if (is.null(names(fold))){
               warning("The list x1 has no names. The elements of the folder are named 'df1', 'df2'...")
               names(fold) <- paste0("df", 1:ndata)
             }
           }
    )
    ...
    if (cols.select == "union") {
      ...
      for (n in 1:ndata) {
        if (ncol(fold[[n]]) > 0) {
          foldn <- merge(data.frame(".rownames" = rownames(fold[[n]]), fold[[n]], stringsAsFactors = TRUE), adjcnames, all = TRUE, sort = FALSE)[1:nrow(fold[[n]]), , drop = FALSE]
