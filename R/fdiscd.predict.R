fdiscd.predict <-
           ...
           crit = 1, windowh = NULL, misclass.ratio = FALSE)
  {
    ...
    moy.pred <- by(as.data.frame(x.pred[, 1:p], stringsAsFactors = TRUE), x.pred[, p+1], colMeans)
    var.pred <- by(as.data.frame(x.pred[, 1:p], stringsAsFactors = TRUE), x.pred[, p+1], var)
    ...
    if (gaussiand) {
      ...
      if (crit == 1)  {
        ...
        switch(choix,
               ...
               uga = {
                 nbL.f <- by(as.data.frame(x.pred[,1:p], stringsAsFactors = TRUE),INDICES=group,FUN=NROW)
                 wL.f <- bandwidth.parameter(p,nbL.f)
                 nbL.g <- by(x[,1:p], INDICES=x[, p+2], FUN=NROW)
                 wL.g <- bandwidth.parameter(p,nbL.g)
                 # Multiplication of the variance by the window parameter
