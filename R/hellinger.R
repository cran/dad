hellinger <- function(x1, x2, check = FALSE)
{
  # Hellinger (Matusita)-inner product between two Gaussian distributions
  #
  # x1, x2: the data.
  ...
  if (p == 1) {
    ...
    x2 <- as.numeric(x2)
    return(hellingerpar(mean(x1),var(x1),mean(x2),var(x2),check=check))
  } else {
    # Multivariate Gaussian densities:
    return(hellingerpar(colMeans(x1),var(x1),colMeans(x2),var(x2),check=check))
  }
}
